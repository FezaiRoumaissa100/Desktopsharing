...
export default function ShareConfig() {
  const [info, setInfo] = useState<{ ip: string; ips: string[]; password: string } | null>(null)
  ...
  const handleStartShare = async () => {
    ...
    try {
      ...
      setInfo({ ip: data.ip, ips: data.ips, password: data.password })
    } catch (e: any) {
      setError(e.message)
    } finally {
      setLoading(false)
    ...
    <div className="flex flex-col items-center justify-center min-h-screen gap-6 bg-gradient-to-br from-green-50 to-green-100">
      ...
      {info && (
        <div className="mt-6 p-6 border rounded-xl bg-green-700 shadow-lg flex flex-col gap-4 min-w-[320px]">
          ...
          <div className="text-sm text-green-200 mt-2">Donnez cette adresse IP (sur le même réseau) et le mot de passe à la personne qui veut accéder à votre écran.</div>
        </div>
      )}
